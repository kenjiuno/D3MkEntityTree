...
bool CCustRichEditCtrl::LoadFromFile(LPCTSTR pszFilePath)
{
	CFile f;
	if (f.Open(pszFilePath, CFile::modeRead|CFile::shareDenyWrite)) {
		EDITSTREAM es;
		es.dwCookie = reinterpret_cast<DWORD>(&f);
		es.dwError = 0;
		es.pfnCallback = fnEditStreamCallback_loadFromFile;
