...
// | D3ModelExplorer : Copyright (c) 2008, kentaro-k.21
// |
...
class CCustRichEditCtrl : public CRichEditCtrl
{
	...
	// 
	static DWORD CALLBACK fnEditStreamCallback_loadFromFile(DWORD nCookie, LPBYTE pbBuff, LONG cbBuff, LONG *pcb)
	{
		CFile *pf = reinterpret_cast<CFile *>(nCookie);
		*pcb = pf->Read(pbBuff, cbBuff);
		return 0;
	}
...
public:
	...
	// 
	bool LoadFromFile(LPCTSTR pszFilePath);
	// 
